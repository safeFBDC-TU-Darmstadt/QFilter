...
// types (AND, OR)) on the lineitem table (only the first four columns are used) for a variable accessibility ratio for the used user group.
public class Experiment11 extends ServerExperiment {

    int[] accessibility = new int[]{10,50,90};
    int numGroups = 2;
    int numServers = 1;
    int numColumns = 4;
    int numRows = 100_000;
    ...
    public Experiment11() throws Exception {
        super();
    }
    ...
    void measureTime() throws Exception {
        for (int accessibilityPercentage : accessibility) {
